...
const startPaymentWorker = () => {
    if (!env.redis.enabled) {
        console.log('Redis is disabled. Payment worker not started.');
        return null;
    }
    const connection = getBullMQConnection();
    if (!connection) {
        console.error('Payment worker: Redis connection unavailable. Exiting.');
        process.exit(1);
    }
    ...
    worker.on('failed', (job, err) => console.error(`Payment job ${job?.id} failed: ${err.message}`));
    worker.on('error', (err) => console.error(`Payment worker error: ${err.message}`));
