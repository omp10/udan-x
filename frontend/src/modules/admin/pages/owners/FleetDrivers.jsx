...
const FleetDrivers = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.ok && json.success) {
          ...
          const fleetDrivers = allDrivers.filter((driver) => driver.owner_id || driver.fleet_id);
  ...
  const filteredDrivers = useMemo(() => {
    return drivers.filter(driver => {
      ...
      const statusMatch = !filters.status || (filters.status === 'approved' ? isApproved(driver) : !isApproved(driver));
                ...
                <tbody className="divide-y divide-gray-200">
                  {isLoading ? (
                    ...
                    pagedDrivers.map((driver) => (
                      <tr key={driver._id} className="bg-white transition-colors hover:bg-gray-50 border-b border-gray-100">
                        ...
                        <td className="px-3 py-4">
                          <button className="text-yellow-500 transition-colors hover:text-yellow-600 bg-yellow-50 p-1.5 rounded-lg">
                            <FileText size={18} fill="currentColor" strokeWidth={1.5} />
                          </button>
                        </td>
                        <td className="px-3 py-4">
                          <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${isApproved(driver) ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
                            {isApproved(driver) ? 'Approved' : 'Pending'}
                          </span>
                        </td>
                        <td className="px-3 py-4">
